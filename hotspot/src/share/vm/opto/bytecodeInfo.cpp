  ...
  _msg(NULL)
{
  ...
  if (_caller_jvms != NULL) {
    ...
    _caller_jvms->set_receiver_info(caller_jvms->receiver_info()); // JDK-8358751
...
bool InlineTree::try_to_inline(ciMethod* callee_method, ciMethod* caller_method,
  ...
  // JDK-8358751: detect direct and indirect recursive inlining for compiled lambda forms
  // (replaces unreliable Node* / uncast() identity across JVMS levels; see OpenJDK fix).
  {
    ...
    ciInstance* lform_callee_recv = NULL;
    ...
    int inline_level = 0;
    for (JVMState* j = jvms; j != NULL && j->has_method(); j = j->caller()) {
      if (j->method() == callee_method) {
        ...
        // compiler stack.
        if (lform_callee_recv != NULL) {
          ciInstance* lform_caller_recv = j->receiver_info();
          assert(lform_caller_recv != NULL || j->depth() == 1 ||
                 (j->caller() != NULL && j->caller()->has_method() &&
                  !j->caller()->method()->is_compiled_lambda_form()), // MH invoker
                 "missing receiver info");
          if (lform_caller_recv == lform_callee_recv || lform_caller_recv == NULL) {
            inline_level++;
          }
        } else {
          inline_level++;
